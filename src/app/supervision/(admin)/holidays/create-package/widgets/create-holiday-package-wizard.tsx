...
type SubmitOutcome = {
  ...
  message: string;
  status?: number;
  /** Full JSON from BFF / upstream (success body or error payload). */
  response: unknown;
};
...
export default function CreateHolidayPackageWizard() {
  ...
  const goNext = () => {
    const err = validateStep(current.id, form);
    if (err) {
      setStepError(err);
      return;
    }
  ...
  const submit = async () => {
    const err = validateStep("review", form);
    if (err) {
      setStepError(err);
      return;
    }
  ...
  if (submitOutcome) {
    ...
    return (
      <Card
        className={cn(
          !submitOutcome.ok && "border-red-200 dark:border-red-900"
        )}
      >
        <CardHeader>
          <CardTitle>
        ...
        <CardContent className="space-y-4">
          ...
          {current.id === "destination" && (
            <div className="grid gap-4 sm:grid-cols-2">
              <FormField
                label="Slug"
                htmlFor="dest-slug"
                hint="eg. mauritius-tour-packages"
              >
                <input
                  id="dest-slug"
          ...
          {current.id === "package" && (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              {(
                [
                  ["pkgId", "Package ID", { hint: "eg. PKG-USA-CLASSIC-001" }],
              ...
              <FormField label="Review count" htmlFor="pkg-reviews">
                <input
                  ...
                  onChange={(e) =>
                    patchPackage({ reviewCount: Number(e.target.value) || 0 })
                  }
                />
              </FormField>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {HOLIDAY_PACKAGE_INCLUSION_OPTIONS.map(({ label }) => {
                  const checked = isInclusionOptionSelected(
                    form.tourPackage.inclusions,
                    label
                  );
                  return (
                    <label
          ...
          {current.id === "itinerary" && (
            <div className="space-y-4">
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                {form.tourPackage.days} day slot{form.tourPackage.days === 1 ? "" : "s"} from
                the Package step. Sort order is set automatically to match the day number.
              </p>
              {form.tourPackage.itinerary.map((day, di) => (
                <div key={di} className="space-y-3 rounded-lg border p-4">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <FormField
                      label="Meals"
                      hint="eg. Breakfast, Lunch, Dinner, None"
                    >
                      <input
                        className={fieldClass}
                        value={day.meals}
    ...
    <div className="space-y-3">
      ...
      <Button
        ...
        onClick={() =>
          onChange([
            ...items,
            { ...emptyItem, sortOrder: items.length + 1 } as T,
          ])
        }
      >
        {addLabel}
