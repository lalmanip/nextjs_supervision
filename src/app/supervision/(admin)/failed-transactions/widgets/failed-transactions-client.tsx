...
import {
  FAILED_BOOKING_RECOVERY_COLUMNS,
  type FailedBookingRecoveryRow,
} from "@/types/failed-booking-recovery";
...
function formatCell(key: string, v: unknown): string {
  ...
  if (
    (key === "itinerary_details" || key === "passenger_details") &&
    s.length > LONG_PREVIEW
  ) {
    return `${s.slice(0, LONG_PREVIEW)}…`;
  }
  return s;
}
...
function buildColumns(rows: FailedBookingRecoveryRow[]): ColumnDef<FailedBookingRecoveryRow>[] {
  ...
  const cols: ColumnDef<FailedBookingRecoveryRow>[] = [];

  for (const { key, label } of FAILED_BOOKING_RECOVERY_COLUMNS) {
  ...
  for (const key of [...present].sort()) {
    cols.push({
      accessorKey: key as keyof FailedBookingRecoveryRow & string,
...
export default function FailedTransactionsClient() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const { data } = await http.get<ApiOk>(
        "/api/supervision/flight-booking/failed-booking-recovery"
      );
      setRows(data.failedBookings ?? []);
      setRawFallback(
        data.failedBookings?.length === 0 && data.raw !== undefined ? data.raw : null
      );
    } catch (e) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            Flight bookings that failed during recovery — live queue from vivapi-user.
          </p>
        </div>
        <Button
          type="button"
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <CardTitle>Failed booking recovery</CardTitle>
          {!loading ? (
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="space-y-3 text-sm text-zinc-600 dark:text-zinc-400">
              <p>No failed booking records returned.</p>
              {rawFallback !== null ? (
            ...
            <DataTable<FailedBookingRecoveryRow, unknown>
              columns={columns}
              data={rows}
              searchPlaceholder="Search failed bookings..."
            />
          )}
        </CardContent>
      </Card>
