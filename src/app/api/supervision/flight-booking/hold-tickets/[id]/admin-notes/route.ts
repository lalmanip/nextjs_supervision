...
export async function PUT(req: Request, ctx: RouteContext) {
  ...
  const upstreamUrl = `${base}/vivapi-user/flight-booking/hold-tickets/${encodeURIComponent(
    id
  )}/admin-notes`;
  ...
  try {
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:PUT hold-tickets admin-notes (upstream)", {
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:PUT hold-tickets admin-notes (upstream response)", {
        endpoint: upstreamUrl,
        status: res.status,
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:PUT hold-tickets admin-notes (error)", {
        endpoint: upstreamUrl,
