...
export async function GET(req: Request) {
  ...
  const upstreamUrl = `${base}/vivapi-user/flight-booking/hold-tickets`;
  ...
  try {
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET hold-tickets (upstream)", {
        endpoint: fullUrl,
        method: "GET",
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET hold-tickets (upstream response)", {
        endpoint: fullUrl,
        status: res.status,
    ...
    if (res.status < 200 || res.status >= 300) {
      const msg =
        ...
        `Hold tickets request failed (${res.status})`;
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET hold-tickets (error)", {
        endpoint: fullUrl,
    ...
    return NextResponse.json(
      {
        message:
          ...
          "message" in (ax.response.data as object)
            ...
            : ax?.message || "Failed to load hold tickets",
      },
      { status: 502 }
    );
  }
