...
export async function GET(req: Request) {
  ...
  const upstreamUrl = `${base}/vivapi-user/flight-booking/cancelled-tickets`;
  ...
  try {
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET cancelled-tickets (upstream)", {
        endpoint: fullUrl,
        method: "GET",
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET cancelled-tickets (upstream response)", {
        endpoint: fullUrl,
        status: res.status,
    ...
    if (res.status < 200 || res.status >= 300) {
      const msg =
        ...
        `Cancelled tickets request failed (${res.status})`;
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET cancelled-tickets (error)", {
        endpoint: fullUrl,
    ...
    return NextResponse.json(
      {
        message:
          ...
          "message" in (ax.response.data as object)
            ...
            : ax?.message || "Failed to load cancelled tickets",
      },
      { status: 502 }
    );
  }
