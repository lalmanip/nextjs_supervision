...
export async function GET(req: Request) {
  ...
  const upstreamUrl = `${base}/vivapi-user/user/b2b/agents`;
  ...
  try {
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b agents (upstream)", {
        endpoint: fullUrl,
        method: "GET",
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b agents (upstream response)", {
        endpoint: fullUrl,
        status: res.status,
    ...
    if (res.status < 200 || res.status >= 300) {
      const msg =
        ...
        `Registered agents request failed (${res.status})`;
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b agents (error)", {
        endpoint: fullUrl,
    ...
    return NextResponse.json(
      {
        message:
          ...
          "message" in (ax.response.data as object)
            ...
            : ax?.message || "Failed to load registered agents",
      },
      { status: 502 }
    );
  }
