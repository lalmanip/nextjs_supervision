...
export async function GET(req: Request) {
  ...
  const upstreamUrl = `${base}/vivapi-user/user/b2b/wallet/requests/pending`;
  ...
  try {
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b wallet pending requests (upstream)", {
        endpoint: fullUrl,
        method: "GET",
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b wallet pending requests (upstream response)", {
        endpoint: fullUrl,
        status: res.status,
    ...
    if (res.status < 200 || res.status >= 300) {
      const msg =
        ...
        `Pending wallet requests failed (${res.status})`;
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b wallet pending requests (error)", {
        endpoint: fullUrl,
    ...
    return NextResponse.json(
      {
        message:
          ...
          "message" in (ax.response.data as object)
            ...
            : ax?.message || "Failed to load pending wallet requests",
      },
      { status: 502 }
    );
  }
