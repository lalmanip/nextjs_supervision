...
export async function GET(req: Request) {
  ...
  const upstreamUrl = `${base}/vivapi-user/user/b2b/pending-activation`;
  ...
  try {
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b pending-activation (upstream)", {
        endpoint: fullUrl,
        method: "GET",
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b pending-activation (upstream response)", {
        endpoint: fullUrl,
        status: res.status,
    ...
    if (res.status < 200 || res.status >= 300) {
      const msg =
        ...
        `Pending activation request failed (${res.status})`;
    ...
    if (isServerApiDebugEnabled()) {
      logApiDebug("route:GET b2b pending-activation (error)", {
        endpoint: fullUrl,
    ...
    return NextResponse.json(
      {
        message:
          ...
          "message" in (ax.response.data as object)
            ...
            : ax?.message || "Failed to load pending activations",
      },
      { status: 502 }
    );
  }
